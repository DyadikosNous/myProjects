...
class InstaDM(object):
    ...
    def __init__(self, username, password, headless=True, instapy_workspace=None, profileDir=None):
        self.selectors = {
            "accept_cookies": "//button[text()='Allow all cookies']",
            ...
            "name": "((//div[@aria-labelledby]/div/span//img[@data-testid='user-avatar'])[1]//..//..//..//div[2]/div["
                    "2]/div)[1]",
        ...
        mobile_emulation = {
            "userAgent": 'Mozilla/5.0 (Linux; Android 4.0.3; HTC One X Build/IML74K) AppleWebKit/535.19 (KHTML, '
                         'like Gecko) Chrome/99.0.4844.51 Mobile Safari/535.19'
        }
    ...
    def login(self, username, password):
        ...
        if not self.__wait_for_element__(self.selectors['username_field'], 'name', 10):
            ...
            self.driver.find_element('name', self.selectors['username_field']).send_keys(username)
            self.driver.find_element('name', self.selectors['password_field']).send_keys(password)
    ...
    def typeMessage(self, user, message):
        ...
        elements = self.driver.find_elements(By.CSS_SELECTOR, '.x1yc6y37')
        if elements and len(elements) > 0:
            elements[0].click()
            self.__random_sleep__()

        notificationBanner = self.driver.find_elements(By.CSS_SELECTOR, '._a9_1')
        notificationBanner[0].click()
        self.__random_sleep__()
    ...
    def sendMessage(self, user, message, greeting=None):
        ...
        try:
            ...
            elements = self.driver.find_elements(By.CSS_SELECTOR, '.xuxw1ft')

            for index, element in enumerate(elements):
                if user in element.text:
    ...
    def __get_element__(self, element_tag, locator):
        ...
        try:
            ...
            if locator == 'ID' and self.is_element_present(By.ID, element_tag):
                ...
                return WebDriverWait(dr, 15).until(lambda d: dr.find_element('name', element_tag))
            elif locator == 'XPATH' and self.is_element_present(By.XPATH, element_tag):
                return WebDriverWait(dr, 15).until(lambda d: dr.find_element('xpath', element_tag))
            elif locator == 'CSS' and self.is_element_present(By.CSS_SELECTOR, element_tag):
                return WebDriverWait(dr, 15).until(lambda d: dr.find_element('css', element_tag))
